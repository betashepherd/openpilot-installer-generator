...
define("WEBSITE_URL", "http://openpilot.bgton.cn");
define("BASE_DIR", "/");
...
list($path, $username, $branch, $loading_msg) = explode("/", $url);  # todo: clip these strings at the max length in index (to show up on the webpage)
...
if (IS_NEOS or IS_AGNOS or IS_WGET) {  # if NEOS or wget serve file immediately. commaai/stock if no username provided
    ...
    header("Location: " . $build_script . "?username=" . $username . "&branch=" . $branch . "&loading_msg=" . $loading_msg);
    return;
}
...
echo '</br></br><a href="/"><h1 style="color: #30323D;">🍴 openpilot fork installer generator-inator 🍴</h1></a>';
...
if ($username == "") {
    ...
    echo "<h2><a href='sshane/SA-master'><span>" . WEBSITE_URL . "/username/branch</span></a></h2>";
...
if (array_key_exists('download_neos', $_POST)) {
    header("Location: /build_neos.php?username=" . $username . "&branch=" . $branch . "&loading_msg=" . $loading_msg);
    exit;
}
if (array_key_exists('download_agnos', $_POST)) {
    header("Location: /build_agnos.php?username=" . $username . "&branch=" . $branch . "&loading_msg=" . $loading_msg);
    exit;
}
?>
